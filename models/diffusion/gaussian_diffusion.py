...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        :param model: the model, which takes a signal and a batch of timesteps
                      as input.
        :param x: the [N x T x D] tensor at time t.
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            assert model_output.shape == (B, nt ,C * 2)
        ...
        if self.model_mean_type == ModelMeanType.START_X:
            ...
            raise NotImplementedError("Model Mean type {} is not supported!".format(self.model_mean_type))
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        :param x_start: the [N x T x D] tensor of inputs.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms["loss_total"] = self._vb_terms_bpd(
                model=model,
                x_start=x_start,
                x_t=x_t,
                t=t,
            ...
            if self.loss_type == LossType.RESCALED_KL:
                terms["loss_total"] *= self.num_timesteps
            ...
            target = {
                ...
                ModelMeanType.VELOCITY: self.get_v(x_start, noise, t)
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start.shape
            terms["loss_mse"] = mean_flat((target - model_output) ** 2)
            if "loss_vb" in terms:
                terms["loss_total"] = terms["loss_mse"] + terms["loss_vb"]
            else:
                terms["loss_total"] = terms["loss_mse"]
        else:
